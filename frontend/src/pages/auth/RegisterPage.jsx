...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(false);
    if (!success) { toast.error(error || "Registration failed"); return; }
    ...
    <div className="min-h-screen bg-[hsl(36,33%,97%)] flex items-center justify-center px-4 py-10">
      ...
      >
        ...
        <div className="bg-white rounded-2xl border border-[hsl(220,18%,91%)] shadow-sm p-8">
          <h1 className="text-2xl font-black text-[hsl(226,64%,14%)] text-center mb-1">Create your account</h1>
          <p className="text-sm text-[hsl(220,14%,50%)] text-center mb-7">Join thousands of students in J&K</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Full Name */}
            <div>
              <label className="block text-xs font-semibold text-[hsl(226,50%,20%)] mb-1.5">Full Name</label>
              <InputField
                icon={User}
                placeholder="Rahul Bhat"
                value={form.name}
            ...
            {/* Password */}
            <div>
              <label className="block text-xs font-semibold text-[hsl(226,50%,20%)] mb-1.5">Password</label>
              <InputField
                icon={Lock}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-[hsl(226,50%,20%)] mb-1.5">Class</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[hsl(226,50%,20%)] mb-1.5">Stream</label>
            ...
            {/* District */}
            <div>
              <label className="block text-xs font-semibold text-[hsl(226,50%,20%)] mb-1.5">District</label>
              <Select onValueChange={(v) => set("district", v)}>
                <SelectTrigger className={selectCls}><SelectValue placeholder="Select your district" /></SelectTrigger>
                <SelectContent>
          ...
          <p className="text-center text-xs text-[hsl(220,14%,50%)] mt-5">
            ...
            <Link to="/login" className="text-[hsl(252,50%,45%)] font-semibold hover:underline">Sign in</Link>
          </p>
        </div>
      </motion.div>
    </div>
