...
export default function ResourcesPage() {
  ...
  if (loading) return <StudentLayout><PageLoader message={t("resources_loading")} /></StudentLayout>;
    ...
    <StudentLayout requireAuth={false}>
      <div className="min-h-full bg-[hsl(36,33%,97%)]">
        <div className="bg-white border-b border-[hsl(220,18%,91%)] px-8 py-5">
          <h1 className="text-2xl font-black text-[hsl(226,64%,14%)]">{t("resources_title")}</h1>
          <p className="text-sm text-[hsl(220,14%,50%)] mt-0.5">{t("resources_subtitle")}</p>
        </div>
        ...
        <div className="px-8 py-6 max-w-[1200px]">
          <div className="flex flex-wrap gap-3 mb-6">
            <div className="relative flex-1 min-w-48">
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t("resources_search")}
            ...
            <select value={typeFilter} onChange={e=>setTypeFilter(e.target.value)}
              ...
              <option value="">{t("resources_all_types")}</option>
              {types.map(typeOpt => <option key={typeOpt} value={typeOpt}>{typeOpt}</option>)}
            </select>
          </div>

          {filtered.length === 0 ? <PageEmpty message={t("resources_no_match")} icon={Library} /> : (
                ...
                <motion.div key={r.id} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      {r.is_free ? <span className="text-[9px] font-bold chip-mint px-2 py-0.5 rounded-full">{t("resources_free")}</span> : null}
                  ...
                  {r.url && (
                    <a href={r.url} target="_blank" rel="noopener noreferrer"
                      ...
                      <ExternalLink className="h-3.5 w-3.5" /> {t("resources_access")}
                    </a>
                  )}
                </motion.div>
              ))}
