...
export default function CourseDetailPage() {
  ...
  if (!course) {
    return (
      <StudentLayout>
        <EmptyState icon={BookOpen} title="Course not found" description="This course doesn't exist" action={<Button onClick={() => navigate("/courses")}>Back to Courses</Button>} className="min-h-screen" />
      </StudentLayout>
    );
  }
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {course.careerAreas.map((a) => (
                    <span key={a} className="text-xs px-2.5 py-1 bg-white/15 rounded-full text-white border border-white/20">{a}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-2xl border border-[hsl(220,18%,91%)] p-6">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
              <BookOpen className="h-5 w-5 text-[hsl(252,50%,45%)]" /> What You'll Study
            </h2>
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-white rounded-2xl border border-[hsl(220,18%,91%)] p-6">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
              <Lightbulb className="h-5 w-5 text-amber-500" /> Skills You'll Develop
            </h2>
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl border border-[hsl(220,18%,91%)] p-6">
            <h2 className="text-lg font-bold mb-6 flex items-center gap-2">
              <TrendingUp className="h-5 w-5 text-teal-600" /> Career Pathway
            </h2>
            {/* Visual flow */}
            <div className="flex flex-col sm:flex-row items-center gap-3 justify-center mb-8">
              {[course.shortName, "Core Skills", "Career Options", "Future Growth"].map((step, i) => (
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="p-4 rounded-xl bg-[hsl(252,60%,96%)] border border-blue-100">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-semibold text-sm text-blue-900">Private Sector Jobs</span>
                </div>
              ...
              <div className="p-4 rounded-xl bg-teal-50 border border-teal-100">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-semibold text-sm text-teal-900">Government Routes</span>
                </div>
                <ul className="space-y-1">
