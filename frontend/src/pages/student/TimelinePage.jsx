...
export default function TimelinePage() {
  ...
  if (loading) return <StudentLayout><PageLoader message="Loading timeline…" /></StudentLayout>;
      ...
      <div className={cn("bg-white border border-[hsl(220,18%,91%)] rounded-xl p-4 flex gap-4", event.computedStatus === 'completed' && 'opacity-60')}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {event.daysLeft >= 0 && event.daysLeft <= 14 && (
            <p className="text-xs font-semibold text-[hsl(22,80%,40%)] mt-1">{event.daysLeft} days remaining</p>
          )}
          {event.category && (
    ...
    <StudentLayout>
      <div className="min-h-full bg-[hsl(36,33%,97%)]">
        <div className="bg-white border-b border-[hsl(220,18%,91%)] px-8 py-5">
          <h1 className="text-2xl font-black text-[hsl(226,64%,14%)]">Admission Timeline</h1>
          <p className="text-sm text-[hsl(220,14%,50%)] mt-0.5">Track important dates and upcoming deadlines in J&K education</p>
        </div>
        ...
        <div className="px-8 py-6 max-w-[900px] space-y-6">
          ...
          {filtered.length === 0 ? <PageEmpty message="No events match your filters." icon={Calendar} /> : (
            <>
              ...
              {groups.completed.length > 0 && (
                <motion.div initial={{opacity:0}} animate={{opacity:1}}>
                  <h2 className="font-bold text-sm text-[hsl(220,14%,55%)] mb-3">Completed</h2>
                  <div className="space-y-3">
