...
export default function ScholarshipsPage() {
  ...
  if (loading) return <StudentLayout><PageLoader message="Loading scholarships…" /></StudentLayout>;
    ...
    <StudentLayout>
      <div className="min-h-full bg-[hsl(36,33%,97%)]">
        <div className="bg-white border-b border-[hsl(220,18%,91%)] px-8 py-5">
          <h1 className="text-2xl font-black text-[hsl(226,64%,14%)]">Scholarships</h1>
          <p className="text-sm text-[hsl(220,14%,50%)] mt-0.5">Scholarships for J&K students · Eligibility computed from your profile</p>
        </div>
        ...
        <div className="px-8 py-6 max-w-[1200px]">
          ...
          {filtered.length === 0 ? <PageEmpty message="No scholarships match your search." icon={Award} /> : (
                ...
                return (
                  <motion.div key={s.id} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}}
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {s.deadline && <div className="text-xs text-[hsl(220,14%,50%)]">Deadline: {s.deadline}</div>}
                    </div>
                    {s.application_url && (
